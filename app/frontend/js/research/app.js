// Generated by scripts/build_research_module.py
export function mountResearch(context) {
  'use strict';

  const { document, window, fetch, setTimeout, clearTimeout, setInterval, clearInterval, requestAnimationFrame, ApexCharts } = context;


  const API_BASE = window.API_BASE || '/research';
...
KOSDAQ|웹젠|게임`,
  ...
  function openTheoryDay(day) {
    ...
    setView('theory');
    renderTheoryDay(THEORY_DAYS.find(lesson => lesson.day === day));
    context.onView('day-' + day);
  }
...
주의: 최신성 확인 필요 · 법률 자문 아님</code></pre></section><footer><i class="fa-solid fa-shield-halved"></i><span>로그인 우회·자동 수집·재배포를 전제로 하지 않습니다. AI Hub 데이터셋 페이지의 이용정책과 개별 라이선스를 우선하고, 개인정보·민감정보는 조건과 내부 정책에 따라 최소화·비식별화합니다.</span><a href="https://aihub.or.kr/" target="_blank" rel="noopener noreferrer">AI Hub 데이터 찾기 <i class="fa-solid fa-arrow-up-right-from-square"></i></a></footer></section>`;
  ...
  async function sendQuestion(question) {
    ...
    try {
      ...
      appendMessage('bot', data.answer.startsWith('LLM 호출에 실패했습니다:') ? '답변 모델이 준비되지 않아 검색한 참고 문서를 표시합니다. 참고 문서에서 원문을 확인해 주세요.' : data.answer, data.references || []);
  ...
  const initialView = ['home', 'stocks', 'learn', 'simulation', 'basis', 'backtest', 'calendar'].includes(requestedView)
    ? requestedView
    : 'home';
  setView(context.initialView === 'documents' ? 'learn' : context.initialView || initialView);
  if (context.initialDay) openTheoryDay(context.initialDay);
  if (context.initialView === 'documents') context.onView('documents');
